...
// ─── Evenement kaart ──────────────────────────────────────────────────────────
function EvenementKaart({ evenement, index }) {
  return (
    <Link
      to={`/evenementen/${evenement.slug}`}
      className="reveal"
      style={{
    ...
    >
      ...
      <div style={{ padding: '2rem 2rem 1.75rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.6rem' }}>
          {[
            { icon: <KalenderIcon />, text: evenement.datum },
            { icon: <KlokIcon />, text: `${evenement.starttijd} – ${evenement.eindtijd}` },
            { icon: <TicketIcon />, text: evenement.prijs },
          ].map(({ icon, text }) => (
            <div key={text} style={{
              display: 'flex',
              alignItems: 'center',
...
function OverzichtHero() {
  ...
  return (
    <section style={{
      ...
      <div style={{ position: 'absolute', inset: 0, background: 'rgba(42, 10, 5, 0.62)' }} />
...
function OverzichtKaarten() {
  ...
  const evenementen = Object.values(evenementenData)
  ...
  return (
    <section ref={sectionRef} style={{
      ...
      <div style={{ position: 'relative', zIndex: 2, maxWidth: '1520px', margin: '0 auto', padding: '0 2.5rem', width: '100%' }}>
        <div className="evenementen-overzicht-grid">
          {evenementen.map((ev, i) => (
            <EvenementKaart key={ev.slug} evenement={ev} index={i} />
          ))}
        </div>
      </div>
    </section>
...
export default function EvenementenOverzicht() {
  return (
    <>
      <OverzichtHero />
      <OverzichtKaarten />
